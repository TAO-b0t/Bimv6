...
export default function AddProjectModal({ isOpen, onClose, onSubmit }) {
  ...
  return (
    ...
    <div className={`bg-[#0f1f23] rounded-lg ${step === 1 ? "w-[600px]" : "w-[1000px]"} p-6 text-white relative`}>
      <button onClick={handleCancel} className="absolute top-4 right-5 text-2xl">×</button>
  
        {step === 1 ? (
          <>
            <h2 className="text-lg font-semibold mb-4 text-center">สร้างโครงการใหม่</h2>
            ...
            <div className="border-2 border-dashed border-gray-500 rounded-lg p-6 text-center cursor-pointer">
              <label htmlFor="upload" className="cursor-pointer">
                {previewImage ? (
                  <img src={previewImage} alt="preview" className="mx-auto h-40 object-contain" />
                ) : (
                  <>
                    <img src={bgUpload} className="mx-auto h-14 mb-2" alt="upload placeholder" />
                    <p className="font-bold">ลากและวางไฟล์ รูปภาพ เพื่ออัปโหลด</p>
                    <p className="text-xs text-gray-400">รูปภาพของคุณจะช่วยให้แสดงรายละเอียดได้ชัดเจน</p>
                  </>
                )}
              </label>
...
>
  ยกเลิก
    ...
    <h2 className="text-center text-lg font-semibold mb-3">เพิ่มผู้ร่วมเข้าโครงการ</h2>
            ...
            <div key={user.email} className="flex items-center  justify-between px-4 py-3">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-gray-400 overflow-hidden">
                  <img src={user.email === currentUser.email ? userImg : ""} alt="avatar" />
                </div>
                <div>
  ...
  <UserRole role={user.user_role} />
    ...
    <UserRole role={user.user_role} />
    <button onClick={() => handleRemove(user)} className="text-gray-500 text-sm hover:text-red-400">×</button>
  </div>
              )}
            </div>
          ))}
        ...
        <div className="max-h-[300px] bg-[#11323E] overflow-y-auto divide-y divide-[#263A3D]">
          {availableUsers.map((user) => (
            <div key={user.email} className="flex items-center justify-between px-4 py-3">
              ...
              <button onClick={() => handleInvite(user)} className="text-xs bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-full">เชิญ</button>
            </div>
          ))}
        </div>
      </div>
